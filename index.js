...
io.on('connection', socket=>{
    ...
    socket.on("newName",data=>{
        for(people of infoServer.peoples){
            if(people.id == data.id){
                console.log("pessoa igual trocar nome ");
                people.nome = data.name;
                socket.me.nome = data.name;
...
server.listen(port|process.env.PORT,console.log("aberto  em https://localhost:"+(port|process.env.PORT)));
